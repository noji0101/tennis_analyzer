...
# TODO 動画を出すかどうかを選択できるようにする？

class Ball_detector():
	def __init__(self, input_video_path, config):
		self.input_video_path = input_video_path
		self.weights_path = config['weights_path']
		self.n_classes = config['n_classes']
		self.is_save_csv = config['is_save_csv']
		self.save_csv_path = config['save_csv_path']
	
	def predict_ball_coordinates(self):
		...
		m = modelFN(self.n_classes , input_height=height, input_width=width)
		...
		while(True):
			...
			pr = pr.reshape((height ,width ,self.n_classes)).argmax( axis=2 )
			...
			if circles is not None:
				...
				if len(circles) == 1:
					...
					x = int(circles[0][0][0])
					y = int(circles[0][0][1])
					ball_coordinates.append([currentFrame, x, y])
		...
		if self.is_save_csv:
			with open(self.save_csv_path, 'w') as f:
				writer = csv.writer(f)
				writer.writerows(ball_coordinates)
		else:
			return np.array(ball_coordinates)

if __name__ == '__main__':
	config = {  'weights_path': 'weights/model.3',
				'n_classes': 256,
				'is_save_csv': True,
				'save_csv_path': './college_3_complete.csv' }
	ball_detector = Ball_detector('./college_3_complete.mp4', config)
	ball_detector.predict_ball_coordinates()
